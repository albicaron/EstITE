...
ACTGdata <- read.csv("./ACTG_ThirdDGP/Data/ACTGdata.csv")
...
system.time(
  ...
  for (i in 1:B) {
    ...
    ############## RESPONSE SURFACES
    mu = 6 + 0.3*(myX[, "wtkg"]^2) - sin(myX[, "age"])*(myX[, "gender"] + 1) +
      0.6*myX[, "hemo"]*myX[, "race"] - 0.2*(myX[, "z30"])
    
    ITE = 1 + 1.5*sin(myX[, "wtkg"])*(myX[, "karnof_hi"]+1) + 0.4*myX[, "age"]^2
...
invisible(
  sapply(names(Results), 
         function(x) write.csv(Results[[x]], 
                               file=paste0(getwd(), "/ACTG_ThirdDGP/Results/ACTG_", B, "_", x, ".csv") ) )
)


write.csv(sapply( names(Results), function(x) colMeans(Results[[x]]) ), 
          file = paste0(getwd(), "/ACTG_ThirdDGP/Results/MeanSummary_", B, ".csv"))

write.csv(sapply( names(Results), function(x) apply(Results[[x]], 2, function(y) MC_se(y, B)) ), 
          file = paste0(getwd(), "/ACTG_ThirdDGP/Results/MCSE_Summary_", B, ".csv"))
